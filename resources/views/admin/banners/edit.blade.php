...
@section('content')
    <div class="space-y-4">
        ...
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-lg font-bold text-slate-900 dark:text-white">Edit Card</h2>
            ...
            <form action="{{ route('admin.cards.update', $banner) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="grid gap-4 lg:grid-cols-2">
                    ...
                    <div>
                        ...
                        <input type="text" name="year" value="{{ old('year', $banner->year) }}"
                            class="mt-2 w-full rounded-xl border border-slate-200 bg-transparent px-4 py-3 text-slate-900 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 dark:border-slate-700 dark:text-white dark:focus:border-blue-500"
                            required>
                        @error('year')
                            <p class="mt-1 text-xs text-rose-600 dark:text-rose-400">{{ $message }}</p>
                    ...
                    <div class="lg:col-span-2">
                        ...
                        <textarea name="description" rows="4"
                            class="mt-2 w-full rounded-2xl border border-slate-200 bg-transparent px-4 py-3 text-slate-900 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 dark:border-slate-700 dark:text-white dark:focus:border-blue-500">{{ old('description', $banner->description) }}</textarea>
                        @error('description')
                            <p class="mt-1 text-xs text-rose-600 dark:text-rose-400">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="lg:col-span-2">
                        ...
                        <div id="bannerImagePreviewWrapper" class="mt-4">
                            <img id="bannerImagePreview"
                                src="{{ $banner->banner_path ? Storage::disk('public')->url($banner->banner_path) : '#' }}"
                                alt="Banner"
                                class="h-44 w-full rounded-2xl object-cover border border-slate-200 dark:border-slate-700 {{ $banner->banner_path ? '' : 'hidden' }}">
                        </div>

                        <input id="banner_image_input" type="file" name="banner_image"
                    ...
                    <div>
                        ...
                        <input type="date" name="start_date"
                            value="{{ old('start_date', $banner->start_time->format('Y-m-d')) }}"
                            class="mt-2 w-full rounded-xl border border-slate-200 bg-transparent px-4 py-3 text-slate-900 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 dark:border-slate-700 dark:text-white dark:focus:border-blue-500"
                            required>
                        @error('start_date')
                            <p class="mt-1 text-xs text-rose-600 dark:text-rose-400">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <input type="time" name="start_time"
                            value="{{ old('start_time', $banner->start_time->format('H:i')) }}"
                            class="mt-2 w-full rounded-xl border border-slate-200 bg-transparent px-4 py-3 text-slate-900 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 dark:border-slate-700 dark:text-white dark:focus:border-blue-500"
                            required>
                        @error('start_time')
                            <p class="mt-1 text-xs text-rose-600 dark:text-rose-400">{{ $message }}</p>
                    ...
                    <!-- Tanggal Berakhir -->
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300">Tanggal
                            Berakhir</label>
                        <input type="date" name="end_date"
                            value="{{ old('end_date', $banner->end_time->format('Y-m-d')) }}"
                            class="mt-2 w-full rounded-xl border border-slate-200 bg-transparent px-4 py-3 text-slate-900 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 dark:border-slate-700 dark:text-white dark:focus:border-blue-500"
                            required>
                        @error('end_date')
                            <p class="mt-1 text-xs text-rose-600 dark:text-rose-400">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <input type="time" name="end_time"
                            value="{{ old('end_time', $banner->end_time->format('H:i')) }}"
                            class="mt-2 w-full rounded-xl border border-slate-200 bg-transparent px-4 py-3 text-slate-900 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 dark:border-slate-700 dark:text-white dark:focus:border-blue-500"
                            required>
                        @error('end_time')
                            <p class="mt-1 text-xs text-rose-600 dark:text-rose-400">{{ $message }}</p>
                ...
                <div
                    class="mt-6 rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-800/50">
                    <label class="flex items-start gap-3 text-sm font-semibold text-slate-700 dark:text-slate-300">
                        <input type="hidden" name="publish" value="0">
                        <input type="checkbox" name="publish" value="1" class="mt-1" @checked(old('publish', $banner->is_published))>
                        <span>
                            Publish card
                            <small class="mt-1 block font-normal text-slate-500 dark:text-slate-400">Matikan untuk menyimpan
                                perubahan sebagai draft.</small>
                        </span>
                    </label>
                </div>
